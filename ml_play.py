...
def ml_loop(side: str):
    ...
    def move_to(player, pred) : #move platform to predicted position to catch ball 
        if player == '1P':
            if scene_info["platform_1P"][0]>pred-15:return 2
            elif scene_info["platform_1P"][0]<pred-15:return 1
            else:return 0
        else :
            if scene_info["platform_2P"][0]>pred-20:return 2
            elif scene_info["platform_2P"][0]<pred-20:return 1
            else:return 0

    def ml_loop_for_1P(): 
        y = clf.predict(feature)
        return move_to(player="1P",pred=y)
        '''pred=gg.predict(a)
    ...
    def ml_loop_for_2P():  # as same as 1P
        '''if scene_info["ball_speed"][1]<0:
            time=(80-scene_info["ball"][1])/scene_info["ball_speed"][1]
